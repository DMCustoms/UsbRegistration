...
public class ConfirmRemovalHandler implements ConfirmHandler {

	private RegisteredItem item;
	private SearchDialog context;
	private ConfirmDialog confirmDialogContext;
	
	public ConfirmRemovalHandler(JDialog context, JDialog confirmDialogContext, RegisteredItem item) {
	...
	public void confirm() {
		try {
			if (Queries.removeQuery(DBConnection.INSTANCE.getStatement(), item)) {
				confirmDialogContext.dispose();
				new ModalDialog(context, DialogMessages.REMOVE_COMPLETE);
				context.setRegisteredItemsList();
			} else {
				new ModalDialog(context, DialogMessages.REMOVE_FAIL);
			}
